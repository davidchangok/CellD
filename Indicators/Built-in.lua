...
-------------------------------------------------
-- aura container (12.1+ 光环容器)
-- 官方安全显示通道: 游戏内部跟踪/过滤/更新光环数据,
-- 插件只做创建时配置(图标/时长文本), 战斗中亦能显示
-- 被 secret 化的光环(12.1 起治疗 HoT/buff 移出 never-secret 名单,
-- 详见蓝贴 "Addons and Auras in Curse of Ula'tek")。
-- 显隐策略: 战斗中(PLAYER_REGEN_DISABLED)显示, 脱战交还现有指示器。
-------------------------------------------------
local auraContainerFrame = CreateFrame("Frame")

local function AuraContainer_InitAuraButton(auraButton)
    auraButton:SetSize(18, 18)

    local icon = auraButton:CreateTexture(nil, "OVERLAY")
...
local auraTrackedSpellIDs

-- 构建追踪列表: 用户自定义指示器(Healers 等)的光环 + 内置 externals
...
function I.CreateAuraContainer(parent)
    if not auraTrackedSpellIDs then
        auraTrackedSpellIDs = AuraContainer_BuildTrackedSpellIDs()
    end
    ...
    container:AddAuraGroup("cellTracked", "HELPFUL", {
        maxFrameCount = 5,
        candidateFilters = {
            includeSpellIDs = auraTrackedSpellIDs,
        },
        ...
        initializeFrame = AuraContainer_InitAuraButton,
    })
    container:Hide()
...
-- 战斗中(光环 secret)显示容器, 脱战交还现有指示器
auraContainerFrame:RegisterEvent("PLAYER_REGEN_DISABLED")
auraContainerFrame:RegisterEvent("PLAYER_REGEN_ENABLED")
auraContainerFrame:SetScript("OnEvent", function(self, event)
