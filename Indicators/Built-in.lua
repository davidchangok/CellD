...
-------------------------------------------------
-- combat aura container (12.1+ 战斗 secret 光环引擎显示, v1.3.1)
-- 12.1 战斗中友方光环对插件不可读, 但官方 AuraContainer 引擎能渲染被
-- secret 化的光环(2026-08-18 游戏内实测)。本模块仅战斗中显示该容器,
-- 脱战隐藏交还现有 Healers 指示器(CellD 原本的图标/设置全部保留)。
-- 按钮样式复刻 CellD 自绘图标: 黑色方形背景 + 图标内缩 1px + CellD 字体。
-- ★ AuraButton 必须完全禁用鼠标(EnableMouse/SetMouseMotionEnabled/
--   SetMouseClickEnabled = false), 否则会拦截 CellD 按钮的点击/悬停施法。
-------------------------------------------------
...
-- 战斗显隐全局事件(所有按钮的 AuraContainer 统一切换)
...
local function AuraContainer_EnsureToggleFrame()
    ...
    AuraContainer_ToggleFrame:SetScript("OnEvent", function(self, event)
        local shown = event == "PLAYER_REGEN_DISABLED"
        F.IterateAllUnitButtons(function(button)
            if button.widgets.aurac then
                if shown then
                    button.widgets.aurac:Show()
                else
                    button.widgets.aurac:Hide()
                end
            end
        end)
    end)
...
function I.CreateAuraContainer(parent)
    ...
    local container = CreateFrame("AuraContainer", nil, parent, "CellDAuraContainerTemplate")
    parent.widgets.aurac = container
    ...
    if pos and pos[1] then
        ...
        container:SetPoint("TOPRIGHT", parent, "TOPRIGHT", 0, 3)
    end
    ...
    -- ★ 容器本身也禁用鼠标, 事件穿透到父按钮(CellD 点击/悬停施法)
    container:EnableMouse(false)

    -- 层级: 血条之上
    container:SetFrameLevel(parent:GetFrameLevel() + 10)

    -- 默认隐藏, 战斗中显示(脱战交还现有指示器)
    container:Hide()
