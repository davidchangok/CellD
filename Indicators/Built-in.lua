...
local function Dispels_SetDispel_Blizzard(self, dispelType)
    self:SetTexture("Interface\\AddOns\\CellD\\Media\\Debuffs\\"..dispelType)
    self:Show()
end

local function Dispels_SetDispel_Rhombus(self, dispelType)
    self:SetTexture("Interface\\AddOns\\CellD\\Media\\Debuffs\\Rhombus")
...
-------------------------------------------------
-- combat aura container (12.1+ 战斗 secret 光环引擎显示, v1.3.2)
-- 12.1 战斗中友方光环对插件不可读, 但官方 AuraContainer 引擎能渲染被
-- secret 化的光环(2026-08-18 实测: 挂 UIParent 独立容器显示正常)。
-- ★ 架构关键: 容器挂 UIParent 独立层(非按钮 child), 位置用 SetPoint
--   相对按钮对齐 —— 这样不进入 secure 按钮(点击/悬停施法靠 attribute
--   安全系统)的子对象链, 鼠标事件穿透到按钮, 点击/悬停施法不受影响。
-- 仅战斗中显示(SetShown 控制), 脱战隐藏交还现有 Healers 指示器。
-- 按钮样式复刻 CellD 自绘图标: 黑色方形背景 + 图标内缩 1px + CellD 字体。
-------------------------------------------------

-- 官方 12.1 secret 名单(never-secret 移除清单)
-- 来源: warcraft.wiki.gg Patch 12.1.0 "Aura Classifications"
local AuraContainerSecretSpells = {
    355941, 363502, 364343, 366155, 367364, 373267, 376788, 409895, -- Preservation Evoker
    360827, 395152, 395296, 410089, 410263, 410686, 413984, -- Augmentation Evoker
    774, 8936, 33763, 48438, 155777, 439530, -- Resto Druid
    17, 194384, 1253593, 1300008, 1300009, -- Disc Priest
    139, 41635, 77489, -- Holy Priest
    115175, 119611, 124682, 450769, 1292922, -- Mistweaver Monk
    974, 383648, 61295, 382024, 207400, 444490, -- Restoration Shaman
    53563, 156322, 156910, 1244893, 200025, 431381, -- Holy Paladin
}

-- 追踪列表: 每次创建重建(布局初始化时序, 缓存会导致法术永久缺失)
local function AuraContainer_BuildTrackedSpellIDs()
    local ids = {}

    -- 1. 官方 secret 名单, IsSpellKnown 过滤(只追踪当前角色已学会的)
    if IsSpellKnown then
        for _, id in ipairs(AuraContainerSecretSpells) do
    ...
    -- 2. Healers(icons/buff)指示器列表
    local layoutTable = Cell.vars and Cell.vars.currentLayoutTable
    if layoutTable and layoutTable.indicators then
        for _, ind in ipairs(layoutTable.indicators) do
            if ind["type"] == "icons" and ind["auraType"] == "buff" and ind["auras"] then
                for k, v in pairs(ind["auras"]) do
                    local n = tonumber(k)
                ...
                for id, v in pairs(spells) do
                    ...
                    if type(v) == "table" then
                        for subId in pairs(v) do
                            if type(subId) == "number" then ids[subId] = true end
                        end
                    end
                end
            end
...
-- 白色时长文本颜色曲线(惰性创建)
local AuraContainer_TimerColorCurve
local function AuraContainer_GetTimerColorCurve()
    if not AuraContainer_TimerColorCurve then
        local curve = CreateColorCurve()
        curve:SetType(Enum.LuaCurveType.Step)
        curve:AddPoint(0, CreateColor(1, 1, 1, 0))
        curve:AddPoint(0.1, CreateColor(1, 1, 1, 1))
        AuraContainer_TimerColorCurve = curve
    end
    return AuraContainer_TimerColorCurve
end

-- AuraButton 初始化(引擎创建按钮时调用, 绑定图标/时长/层数/样式)
local function AuraContainer_InitAuraButton(auraButton, config)
    auraButton:SetSize(config.size[1], config.size[2])

    -- CellD 图标样式: 黑色方形背景(引擎按钮不支持 SetBackdrop 则跳过)
    if auraButton.SetBackdrop then
        auraButton:SetBackdrop({ bgFile = Cell.vars.whiteTexture })
        auraButton:SetBackdropColor(0, 0, 0, 1)
    end

    auraButton:SetIcon(auraButton.IconTexture)

    -- 时长文本(跟随 Healers showDuration/showFont 配置)
    if config.showDuration then
        local formatter = CreateNumericRuleFormatter()
        formatter:SetBreakpoints({
            { threshold = 60, format = "%.0f" },
            { threshold = 3, format = "%.0f" },
            { threshold = 0.1, format = "%.1f" },
        })
        auraButton:SetDurationText(auraButton.TimerText, {
            textFormatter = formatter,
            textColor = {
                curve = AuraContainer_GetTimerColorCurve(),
                property = Enum.DurationTextBindingProperty.RemainingDuration,
            },
        })
        if config.font and config.font[1] then
            local f = F.GetFont(config.font[1][1])
            auraButton.TimerText:SetFont(f, config.font[1][2] or 11, "OUTLINE")
        end
        auraButton.TimerText:SetTextColor(1, 1, 1)
        auraButton.TimerText:ClearAllPoints()
        auraButton.TimerText:SetPoint(config.font and config.font[1][5] or "TOPRIGHT", config.font and config.font[1][6] or 2, config.font and config.font[1][7] or 1)
    else
        auraButton:ClearDurationText()
    end

    -- 层数(跟随 Healers showStack/showFont 配置)
    if config.showStack then
        auraButton:SetApplicationCount(auraButton.CountText)
        if config.font and config.font[2] then
            local f = F.GetFont(config.font[2][1])
            auraButton.CountText:SetFont(f, config.font[2][2] or 11, "OUTLINE")
        end
        auraButton.CountText:SetTextColor(1, 1, 1)
        auraButton.CountText:ClearAllPoints()
        auraButton.CountText:SetPoint(config.font and config.font[2][5] or "BOTTOMRIGHT", config.font and config.font[2][6] or 2, config.font and config.font[2][7] or -1)
    else
        auraButton:ClearApplicationCount()
    end

    -- ★ 鼠标完全穿透: 禁止引擎按钮拦截鼠标, 保证 CellD 点击/悬停施法正常
    auraButton:EnableMouse(false)
    auraButton:SetMouseMotionEnabled(false)
    auraButton:SetMouseClickEnabled(false)
end

-- 容器注册表: [container] = button
local AuraContainer_AllContainers = {}

-- 刷新单个容器显隐: 仅战斗且按钮可见时显示
local function AuraContainer_RefreshShown(container)
    local button = AuraContainer_AllContainers[container]
    if InCombatLockdown() and button and button:IsShown() then
...
-- 全局战斗状态切换(所有容器统一刷新)
local AuraContainer_ToggleFrame
local function AuraContainer_EnsureToggleFrame()
    if AuraContainer_ToggleFrame then
        return
    end

    AuraContainer_ToggleFrame = CreateFrame("Frame")
    AuraContainer_ToggleFrame:RegisterEvent("PLAYER_REGEN_DISABLED")
    AuraContainer_ToggleFrame:RegisterEvent("PLAYER_REGEN_ENABLED")
    AuraContainer_ToggleFrame:SetScript("OnEvent", function()
        for container in pairs(AuraContainer_AllContainers) do
            AuraContainer_RefreshShown(container)
        end
    end)
end

function I.CreateAuraContainer(button)
    -- 样式基准: 布局中第一个 icons/buff 指示器(Healers)
    local config = {
        size = {13, 13},
        num = 5,
        spacing = {0, 0},
    ...
    if layoutTable and layoutTable.indicators then
        for _, ind in ipairs(layoutTable.indicators) do
            if ind["type"] == "icons" and ind["auraType"] == "buff" then
                if ind["size"] then config.size = ind["size"] end
                if ind["num"] then config.num = ind["num"] end
                if ind["numPerLine"] then config.numPerLine = ind["numPerLine"] end
                if ind["spacing"] then config.spacing = ind["spacing"] end
                if ind["font"] then config.font = ind["font"] end
                if ind["position"] then config.position = ind["position"] end
                if ind["orientation"] then config.orientation = ind["orientation"] end
                config.showStack = ind["showStack"] ~= false
                config.showDuration = ind["showDuration"] == true
                break
            end
        end
    end

    local tracked = AuraContainer_BuildTrackedSpellIDs()

    -- ★ 容器挂 UIParent 独立层(非按钮 child, 避免 secure 按钮鼠标冲突)
    local container = CreateFrame("AuraContainer", nil, UIParent, "CellDAuraContainerTemplate")

    -- 位置: SetPoint 相对按钮对齐(按钮移动/缩放时自动跟随)
    local pos = config.position
    if pos and pos[1] then
        local rel = pos[2] == "button" and button or pos[2]
        container:SetPoint(pos[1], rel, pos[3], pos[4] or 0, pos[5] or 0)
    else
        container:SetPoint("TOPRIGHT", button, "TOPRIGHT", 0, 3)
    end

    -- 层级: 与按钮同 strata, frameLevel 高于按钮(覆盖显示在按钮之上)
    container:SetFrameStrata(button:GetFrameStrata())
    container:SetFrameLevel(button:GetFrameLevel() + 10)

    -- 布局(方向跟随 Healers, 元素大小 = Healers 图标大小)
    local layout = {
        layoutAxis = AnchorUtil.FlowLayoutAxis.Horizontal,
        horizontalDir = (config.orientation == "left-to-right") and AnchorUtil.FlowDirection.Right or AnchorUtil.FlowDirection.Left,
        elementWidth = config.size[1],
        elementHeight = config.size[2],
        elementSpacing = config.spacing[1] or 0,
    }

    container:AddAuraGroup("cellTracked", "HELPFUL", {
        maxFrameCount = config.num or 5,
        candidateFilters = {
            includeSpellIDs = tracked,
        },
        sortMethod = AuraContainerSortMethod.Expiration,
        sortDirection = AuraContainerSortDirection.Normal,
        layout = layout,
        templateNames = { "CellDAuraButtonTemplate" },
        initializeFrame = function(auraButton)
            AuraContainer_InitAuraButton(auraButton, config)
        end,
    })

    -- ★ 容器本身也禁用鼠标, 事件穿透到下方按钮(CellD 点击/悬停施法)
    container:EnableMouse(false)
    container:SetMouseMotionEnabled(false)

    -- 绑定当前单位(若有); 单位变化由 UnitButton_OnAttributeChanged 同步
    if button.states and button.states.unit then
        container:SetUnit(button.states.unit)
    end

    -- 引擎常启用, 显隐由 SetShown 控制(战斗中显示/脱战隐藏)
    container:SetEnabled(true)
    container:SetShown(false)

    -- 注册 + 按钮显隐同步(容器挂 UIParent, 按钮 Hide 时容器也要隐藏)
    AuraContainer_AllContainers[container] = button
    button.auracContainer = container
    ...
    AuraContainer_EnsureToggleFrame()
...
-- role icon
-------------------------------------------------
local ICON_PATH = "Interface\\AddOns\\CellD\\Media\\Roles\\"
...
local READY_CHECK_STATUS = {
    ready = {t = "Interface\\AddOns\\CellD\\Media\\Icons\\readycheck-ready", c = {0, 1, 0, 1}},
    waiting = {t = "Interface\\AddOns\\CellD\\Media\\Icons\\readycheck-waiting", c = {1, 1, 0, 1}},
    notready = {t = "Interface\\AddOns\\CellD\\Media\\Icons\\readycheck-notready", c = {1, 0, 0, 1}},
}
...
function I.CreatePowerWordShield(parent)
    ...
    function powerWordShield:SetShape(shape)
        local tex = "Interface\\AddOns\\CellD\\Media\\Shapes\\"..shape.."_filled.tga"
...
function I.CreateCombatIcon(parent)
    ...
    combatIcon.tex:SetTexture("Interface\\AddOns\\CellD\\Media\\Icons\\combat", nil, nil, "TRILINEAR")
    ...
    combatIcon.flashTex:SetTexture("Interface\\AddOns\\CellD\\Media\\Icons\\combat_glow", nil, nil, "TRILINEAR")
