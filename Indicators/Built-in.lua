...
-------------------------------------------------
-- aura container (12.1+ 统一光环显示, v1.3.0)
-- 官方光环容器: 引擎内建光环追踪/过滤/倒计时渲染, 战斗中亦能显示
-- 被 secret 化的治疗 HoT/buff(2026-08-18 游戏内实测: AuraContainer 引擎
-- C++ 侧独立渲染, 插件读不到光环数据不影响引擎显示, 推翻 v1.1.0
-- "容器同样受限"的源码推断结论)。
-- filter 用纯 "HELPFUL"(实测 RAID_IN_COMBAT 非必需且有副作用:
-- HELPFUL|RAID_IN_COMBAT 脱战只显示激流, 纯 HELPFUL 显示全部光环)。
-- 常驻显示(脱战/战斗统一引擎渲染), 替代 SecretAuraTracker 施放追踪。
-- 样式基准 = 布局中 icons/buff 指示器(Healers)配置: 位置/大小/方向/字体。
-------------------------------------------------

-- 官方 12.1 secret 名单(never-secret 移除清单), 与 SecretAuraTracker 一致
...
-- 追踪列表: 每次创建重建(布局初始化时序可能导致列表缺失, 不缓存)
...
-- AuraButton 初始化(引擎创建按钮时调用, 绑定图标/时长/层数)
local function AuraContainer_InitAuraButton(auraButton, config)
    ...
    if config.showDuration then
        -- 降序 breakpoints(语义: 第一个 threshold <= duration 的规则生效)
        ...
        auraButton:SetDurationText(auraButton.TimerText, {
            ...
            textColor = {
                curve = Cell.vars.auraContainerTimerColorCurve,
...
function I.CreateAuraContainer(parent)
    ...
    -- 白色时长文本颜色曲线(引擎绑定用, 首次惰性创建)
    if not Cell.vars.auraContainerTimerColorCurve then
        local curve = CreateColorCurve()
        curve:SetType(Enum.LuaCurveType.Step)
        curve:AddPoint(0, CreateColor(1, 1, 1, 0))
        curve:AddPoint(0.1, CreateColor(1, 1, 1, 1))
        Cell.vars.auraContainerTimerColorCurve = curve
    end
